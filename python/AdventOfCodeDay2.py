...
test1="""798	1976	1866	1862	559	1797	1129	747	85	1108	104	2000	248	131	87	95
...
testsplit = test1.split("\n")

print("testsplit: {0}".format(testsplit))

length = len(testsplit)
...
for x in testsplit:
    testsplit2 = x.split("\t")
    matrix.append([int(x) for x in testsplit2])

print("matrix: {0}".format(matrix))
for i in matrix:
    print("i: {0}".format(list(i)))
    for x in list(i):
        print("x: {0}".format(int(x)))
        newList = list(i)
        print("i: {0}".format(i))
        ...
        for t in newList:
            ...
            if res1 == 0 and (int(t) / int(x)) != 1:
                checksum = checksum + (int(t) / int(x))
            res2 = x % t
            if res2 == 0 and (int(t) / int(x)) != 1:
                checksum = checksum + (int(x) / int(t))
print("checksum: {0}".format(checksum / 2))
        # te = list(filter(lambda z: z % x == 0, i))
        # print("te: {0}".format(te))


# for i in range(0, len(matrix)):
#     intlist = map(int, matrix[i])
#     print("intlist: {0}".format(intlist))
#     numpyarray = numpy.array(intlist)
#     print(numpyarray)
#     for n in intlist:
#         remainds = numpy.mod(numpyarray, 1)
#         print(remainds)
#     # minimumval = min(intlist)
#     # maximumval = max(intlist)
#     # difference = maximumval - minimumval
#
